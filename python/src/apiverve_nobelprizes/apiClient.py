...
class NobelprizesAPIClientError(Exception):
...
class ValidationError(NobelprizesAPIClientError):
...
class NobelprizesAPIClient:
    ...
    VALIDATION_RULES = {"firstname": {"type": "string", "required": False}, "lastname": {"type": "string", "required": False}, "category": {"type": "string", "required": False}, "year": {"type": "integer", "required": False, "min": 1901, "max": 2030}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the NobelprizesAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises NobelprizesAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise NobelprizesAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[NobelprizesAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns NobelPrizesData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises NobelprizesAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise NobelprizesAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise NobelprizesAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise NobelprizesAPIClientError(f"Request failed: {str(e)}")
